...
if(flowFile != None):
  ...
  sqltemplate = "insert into tab (key, value) values ('{}','{}');"
  outlines = [ sqltemplate.format( successRate[0].strip() , successRate[1].strip() ) ]
  outlines.append( sqltemplate.format( avgDuration[0].strip() , (":".join(list(map(lambda x: x.strip(), avgDuration[1:])))) ) )
  outlines.append( sqltemplate.format( overwriteRate[0].strip() , overwriteRate[1].strip() ) )
  global outputs
  outputs = "\n".join(outlines)
  # write
